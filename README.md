# 压缩折角网格斜激波问题研究
[Readme (HTML format)](README.html)
## 项目概述
...
这里$U_{\text{out}}$ 是边界上的流体属性，而 $U_{\text{out}-1}$ 和 $U_{\text{out}-2}$ 是靠近边界的内部网格点的对应属性值。
...
$$ U_{ij}^{n+1} = U_{ij}^n - \frac{\Delta t}{A_{ij}} \left( \frac{\partial E^{'}_{i+\frac{1}{2},j}}{\partial \xi} [L_{i\pm\frac{1}{2},j}] + \frac{\partial F^{'}_{i,j+\frac{1}{2}}}{\partial \eta} [L_{i,j\pm\frac{1}{2}}] \right) $$

其中通量向量 $ E^{'}_{i+\frac{1}{2},j} $ 和 $ F^{'}_{i,j+\frac{1}{2}} $ 以及耗散项 $ D $ 定义如下：

$$ \frac{\partial E^{'}_{i+\frac{1}{2},j}}{\partial \xi} [L_{i\pm\frac{1}{2},j}] = (E_{i+1} - D_{i+1}(U_{i+1} - U_{i}))[L_2] + (E_{i} + D_{i}(U_{i} - U_{i-1}))[L_4] \\
E_{i} = E_{i}n_{x} + F_{i}n_{y} $$
...
引入新项 $c$ 代表正常声速，其中 $\epsilon$ 是人工耗散系数。F方向上同理。

#### 修正器
修正器方程用于校正单元格在下一个时间步的实际状态变量 $U_{ij}^{n+1}$，该方程如下所示：

$$ U_{ij}^{n+1} = \frac{1}{2} \left[ U_{ij}^n + U_{ij}^{n+1} \right] - \frac{\Delta t}{A_{ij}} \left( \frac{\partial E_{i-\frac{1}{2},j}^{''}}{\partial \xi} \left[ L_{i \pm \frac{1}{2},j} \right] + \frac{\partial F_{i,j - \frac{1}{2}}^{''}}{\partial \eta} \left[ L_{i,j \pm \frac{1}{2}} \right] \right) $$

其中 $A_{ij}$ 是单元格的面积，而 $\Delta t$ 是时间步长。

通量向量 $E_{i-\frac{1}{2},j}^{''}$ 和 $F_{i,j-\frac{1}{2}}^{''}$ 的扩展包括对流项和耗散项：

$$ \frac{\partial E_{i\pm\frac{1}{2},j}^{''}}{\partial \xi} \left[ L_{i-\frac{1}{2},j} \right] = \left( E_{i} - D_{i}(U_{i+1} - U_{i}) \right) [L_2] + \left( E_{i-1} + D_{i-1}(U_{i} - U_{i-1}) \right) [L_4] $$
