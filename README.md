...
$$
\frac{\partial {E}^*_{i+\frac{1}{2},j}}{\partial \xi} [L_{i\pm\frac{1}{2},j}] = ({E}_{i+1} - D_{i+1}(U_{i+1} - U_{i}))[L_2] + ({E}_{i} + D_{i}(U_{i} - U_{i-1}))[L_4] \\
{E}_{i} = {E}_{i}n_{x} + {F}_{i}n_{y}
$$
...
$$
U_{ij}^{n+1} = \frac{1}{2} \left[ U_{ij}^n + U_{ij}^{n+1} \right] - \frac{\Delta t}{A_{ij}} \left( \frac{\partial {E}_{i-\frac{1}{2},j}^{**}}{\partial \xi} \left[ L_{i \pm \frac{1}{2},j} \right] + \frac{\partial {F}_{i,j - \frac{1}{2}}^{**}}{\partial \eta} \left[ L_{i,j \pm \frac{1}{2}} \right] \right)
$$

其中 $A_{ij}$ 是单元格的面积，而 $\Delta t$ 是时间步长。

通量向量 ${E}_{i-\frac{1}{2},j}^{**}$ 和 ${F}_{i,j-\frac{1}{2}}^{**}$ 的扩展包括对流项和耗散项：

$$
\frac{\partial {E}_{i\pm\frac{1}{2},j}^{**}}{\partial \xi} \left[ L_{i-\frac{1}{2},j} \right] = \left( {E}_{i} - D_{i}(U_{i+1} - U_{i}) \right) [L_2] + \left( {E}_{i-1} + D_{i-1}(U_{i} - U_{i-1}) \right) [L_4]
$$
